...
async def actors_directory_page(req):
    ...
    for a in all_actors:
        ...
        act_items += f'<div class="act-card card-enter" onclick="window.location.href=\'/actor/{str(a["_id"])}\'"><div style="position:relative; padding-top:135%; background:var(--bg3); overflow:hidden; border-radius:10px;"><img src="/api/actor/photo?id={str(a["_id"])}&v={v}" class="act-poster" loading="lazy"><div style="position:absolute; top:8px; left:8px; background:rgba(0,0,0,0.8); border:1px solid rgba(255,255,255,0.1); color:#fff; font-size:9px; padding:4px 8px; border-radius:4px; font-weight:800; backdrop-filter:blur(4px); z-index:2;">{i} {cat.capitalize()}</div><div style="position:absolute; bottom:0; left:0; right:0; background:linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 60%, transparent 100%); padding:20px 8px 8px 8px; z-index:2;"><div style="font-size:12px; font-weight:800; color:#fff; text-overflow:ellipsis; overflow:hidden; white-space:nowrap; text-shadow:0 1px 4px rgba(0,0,0,0.8);">{html.escape(a.get("name", ""))}</div></div></div></div>'
...
async def api_directory_search(req):
    ...
    if mode == "text":
        ...
        for a in docs:
            ...
            i = "🎭" if c == "actor" else "📱" if c == "app" else "🌐"
            v = int(a.get("photo_updated_at") or a.get("created_at") or 0)
            html_out += f'''<div class="act-card card-enter" onclick="window.location.href=\'/actor/{str(a["_id"])}\'"><div style="position:relative; padding-top:135%; background:var(--bg3); overflow:hidden; border-radius:10px;"><img src="/api/actor/photo?id={str(a["_id"])}&v={v}" class="act-poster" loading="lazy"><div style="position:absolute; top:8px; left:8px; background:rgba(0,0,0,0.8); border:1px solid rgba(255,255,255,0.1); color:#fff; font-size:9px; padding:4px 8px; border-radius:4px; font-weight:800; backdrop-filter:blur(4px); z-index:2;">{i} {c.capitalize()}</div><div style="position:absolute; bottom:0; left:0; right:0; background:linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 60%, transparent 100%); padding:20px 8px 8px 8px; z-index:2;"><div style="font-size:12px; font-weight:800; color:#fff; text-overflow:ellipsis; overflow:hidden; white-space:nowrap; text-shadow:0 1px 4px rgba(0,0,0,0.8);">{html.escape(a.get("name", ""))}</div></div></div></div>'''
